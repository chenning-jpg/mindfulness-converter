...
const Navigation: React.FC<NavigationProps> = ({ currentView, setView, inventory, userEmail, onSignOut }) => {
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <aside className={`
        fixed left-0 top-0 bottom-0 z-50 flex flex-col w-[260px] bg-[#0d0d0d] text-[#ececec] border-r border-white/5 shadow-2xl transition-transform duration-300 ease-in-out
      ...
      `}>
        ...
        <div className="absolute top-2 right-2 lg:hidden z-50">
          <button
            onClick={() => setIsMobileMenuOpen(false)}
            className="p-2 text-white/40 hover:text-white bg-black/20 rounded-full"
          >
            <X size={20} />
          </button>
        </div>
        ...
        <div className="p-3 mb-2 mt-8 lg:mt-0">
          <button
